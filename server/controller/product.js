...
export const updateProduct = TryCatch(async(req,res) => {
    ...
    const updatedProduct = await Product.findByIdAndUpdate(req.params.id, updateFields,
        ...
        res.json({
            Message: "Produk telah diperbarui", updatedProduct,
        });
});
...
export const getAllProductsAdmin = TryCatch(async (req, res) => {
    if (req.user.role !== "admin") {
      return res.status(403).json({ message: "Anda bukan admin" });
    }
  
    const products = await Product.find().sort("-createdAt"); // tanpa limit
